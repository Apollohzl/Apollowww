...
async function searchBooks() {
    ...
    try {
        ...
        // 发起搜索请求
        const response = await fetch(`http://101.35.133.34:5000/api/search?key=${searchTerm}`);
...
async function getChapterContent(itemId) {
    ...
    try {
        const response = await fetch(`http://101.35.133.34:5000/api/content?tab=小说&item_id=${itemId}`);
        const data = await response.json();
